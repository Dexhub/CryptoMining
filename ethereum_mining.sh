...
# parsing command line options
    ...
    do
        ...
        -h)   printf "\n%s\n\n%s\n%s\n%s\n%s\n%s\n\n%s\n\n%s\n\n" "--------- ethereum_mining.sh help menu ---------" \
              ...
              "-o       overclocking only" \
              "example usage:" "sudo ethereum_mining.sh -v" 1>&3 2>&4
              exit 1
              ;;
        -v)   exec 1>&3
              exec 2>&4
...
# setting up permissions and files for automated second and/or third run
    ...
    then
        ...
        cp "$(readlink -f $0)" /usr/local/sbin/ethereum_mining.sh
        chmod a+x /usr/local/sbin/ethereum_mining.sh
        if [ -d "/home/${user_array[0]}/.config/autostart/" ] || mkdir -p "/home/${user_array[0]}/.config/autostart/"
        then
             printf "%s\n%s\n%s\n%s" "[Desktop Entry]" "Name=eth" \
             "Exec=sudo /usr/bin/gnome-terminal -e /usr/local/sbin/ethereum_mining.sh" \
...
# Test for 60 minutes
    ...
    then
         ...
         printf "%s\n" "This is a stability check, it will automatically end after 60 minutes"
         touch /.test_complete
         read -d "\0" -a user_array < <(who)
         rm -rf /setupethminer
...
# Automatic startup with provided wallet address
    ...
    then
       ...
       ethminer -U -F "http://eth-us.dwarfpool.com:80/$wallet"
       if [ "$?" -eq 0 ]
       then
       systemctl reboot
       else
